...
export const TradingChartsSecurity: React.FC<Props> = ({
  securityLastInfo,
  start,
  layout,
}) => {
  ...
  const [start1, setStart1] = useState<Date>(
    start
      ? moment(start)
          .subtract(30, "days")
          .hours(9)
  ...
  const [start2, setStart2] = useState<Date>(
    start
      ? moment(start)
          .subtract(15, "days")
          .hours(9)
  ...
  const [start3, setStart3] = useState<Date>(
    start
      ? moment(start)
          .subtract(1, "days")
          .hours(9)
  ...
  const [start4, setStart4] = useState<Date>(
    start ? moment(start).subtract(1, "hours").toDate() : null
  );
  ...
  const updateSize = () => {
    ...
    setChart2Width(
      chart2Ref.current
        ? chart2Ref.current.clientWidth - offset
        : CHART_MIN_WIDTH
    );
    setChart3Width1(
      chart3Ref1.current
    ...
    setChart3Width(
      chart3Ref.current
        ? chart3Ref.current.clientWidth - offset
        : CHART_MIN_WIDTH
    );
  };
  ...
  const getTrend = (interval: Interval): Trend => {
    return premise?.analysis?.trends.find(
  ...
  return (
    <div
      id={"trading-chart-security-" + securityLastInfo.id}
      className="p-grid sample-layout analysis"
    >
          ...
          <div className="p-col-12">
            ...
            <div className="p-grid">
              ...
              >
                <TrendViewChart
                  key={timeFrame1}
                  trend={premise?.analysis?.trends?.find(
                    (value) => value.interval === timeFrame1
                  )}
                  srLevels={premise?.analysis?.srLevels}
                  width={chart1Width1}
                  height={600}
              ...
              >
                <TrendViewChart
                  key={timeFrame2}
                  trend={premise?.analysis?.trends?.find(
                    (value) => value.interval === timeFrame2
                  )}
                  srLevels={premise?.analysis?.srLevels}
                  width={chart2Width1}
                  height={600}
              ...
              >
                <TrendViewChart
                  key={timeFrame3}
                  trend={premise?.analysis?.trends?.find(
                    (value) => value.interval === timeFrame3
                  )}
                  srLevels={premise?.analysis?.srLevels}
                  width={chart3Width1}
                  height={600}
