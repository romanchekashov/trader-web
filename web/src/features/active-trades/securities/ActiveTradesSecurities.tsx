...
export const ActiveTradesSecurities: React.FC<Props> = ({
  securities,
  selectedSecurity,
  onSelectRow,
}) => {
  const columns = [
    ...
    {
      field: "distancePassedSinceLastDayCloseRelativeToAtr",
      header: "GAtrDis (D)%",
    },
  ...
  const signalsTemplate = (rowData, column) => {
    if (rowData.signals && rowData.signals.length > 0) {
      return (
        <div className="securities-candles">
  ...
  const demandSupplyTemplate = (rowData, column) => {
    return (
      <DemandSupply
        totalDemand={rowData.totalDemand}
    ...
    return (
      <div
        style={{
          overflowX: "hidden",
          fontWeight:
            SecurityType.CURRENCY === rowData.type ? "bold" : "normal",
        }}
      >
        {rowData.type}
      </div>
    ...
    if ("totalDemand" === col.field) {
      ...
      return (
        <Column
          ...
          field={col.field}
          header={col.header}
          body={valueTodayTemplate}
          sortFunction={valueTodaySort}
          style={{ width: "70px" }}
        />
      );
      ...
      return (
        <Column
          ...
          field={col.field}
          header={col.header}
          body={volumeTodayTemplate}
          sortFunction={volumeTodaySort}
          style={{ width: "70px" }}
        />
      );
