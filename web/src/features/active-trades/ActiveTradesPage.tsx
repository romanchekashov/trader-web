...
export const ActiveTradesPage = () => {
  ...
  const [selectedSecurity, setSelectedSecurity] = useState<SecurityLastInfo>(
    null
  );
  ...
  useEffect(() => {
    activeTradesApi.getActiveTrades().then(updateActiveTrades);

    const lastSecuritiesSubscription = WebsocketService.getInstance()
      .on<SecurityLastInfo[]>(WSEvent.LAST_SECURITIES)
      .subscribe((securities) => {
        securities = securities.filter(
          (sec) => activeTradesSecIds.indexOf(sec.id) > -1
        );
        securities = activeTradesSecIds.map((id) =>
          securities.find((sec) => sec.id === id)
        );
  ...
  const updateActiveTrades = (activeTrades: ActiveTrade[]): void => {
    ...
    if (
      activeTrades.some((at) => activeTradesSecIds.indexOf(at.secId) === -1)
    ) {
