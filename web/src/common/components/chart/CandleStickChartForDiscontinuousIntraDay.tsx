...
> {
  ...
  static getDerivedStateFromProps = (props: Props, state: State) => {
    ...
    if (props.trends !== state.trends_1) {
      ...
      if (!update) {
        for (let i = 0; i < trends.length; i++) {
          const trendStartEndSum =
            trends_1[i].start[1] +
            trends_1[i].end[1] +
            trends_1[i].start[0] +
            trends_1[i].end[0];
          const trendLineStartEndSum =
            trends[i].start[1] +
            trends[i].end[1] +
            trends[i].start[0] +
            trends[i].end[0];
    ...
    const { orders, stops, possibleTrade } = props;

    const diffInteractiveLengths =
      orders &&
      stops &&
    ...
    const hasOrdersNotInInteractiveMap =
      orders &&
      orders.some((order) => !state.interactiveOrderMap[order.orderNum]);

    const hasStopsNotInInteractiveMap =
      stops && stops.some((stop) => !state.interactiveOrderMap[stop.transId]);

    const hasPossibleTradeInInteractiveMap =
      possibleTrade &&
      !interactivePossibleTrade.interactiveMapContainsPossibleTrade(
        state.interactiveOrderMap,
        possibleTrade
      );

    if (
      diffInteractiveLengths ||
      hasOrdersNotInInteractiveMap ||
      hasStopsNotInInteractiveMap ||
      hasPossibleTradeInInteractiveMap
    ) {
      return CandleStickChartForDiscontinuousIntraDay.getInteractiveOrderMap(
        orders,
        stops,
        possibleTrade,
        state.yCoordinateList_1
      );
    }
  ...
  static getInteractiveOrderMap = (
    ...
    if (orders?.length) {
      interactiveOrder.fillInteractiveMap(interactiveOrderMap, orders);
    }

    if (stops?.length) {
      interactiveStop.fillInteractiveMap(interactiveOrderMap, stops);
    }

    if (possibleTrade) {
      interactivePossibleTrade.fillInteractiveMap(
        interactiveOrderMap,
        possibleTrade
      );
    }
        ...
        if (trends_1.some((each) => each.selected)) {
          needSave({
            save: trends_1
              ...
              .map(this.mapChartTrendToTrendLine),
            delete: trends_1
              .filter((each) => each.selected)
              .map(this.mapChartTrendToTrendLine),
          });
  ...
  handleChoosePosition = (alert, moreProps, e) => {
    ...
    this.setState({
      yCoordinateList_1: [...this.state.yCoordinateList_1, alert],
    });
    onEnableNewOrder(false);
  };
      ...
      <div id={this.id} onClick={(e) => setCurrentChartInteractingId(this.id)}>
        ...
        >
          ...
          >
            ...
            <CandlestickSeries
              fill={(d) => (d.close > d.open ? "#ecf0f1" : "#000")}
              stroke="#000"
              wickStroke="#000"
            />
            <EdgeIndicator
            ...
            {candlePatternsUp ? (
              <ScatterSeries
                ...
                marker={Square}
                markerProps={{ width: 16, stroke: "#43a047", fill: "#43a047" }}
              />
            ) : null}
            ...
            {candlePatternsDown ? (
              <ScatterSeries
                ...
                marker={Square}
                markerProps={{ width: 16, stroke: "#e53935", fill: "#e53935" }}
              />
            ) : null}
