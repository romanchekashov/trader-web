...
const MarketState: React.FC<Props> = ({
  filter,
  initMarketState,
  viewHeight,
}) => {
  const [marketStateIntervals, setMarketStateIntervals] = useState<
    MarketStateInterval[]
  >([]);
    ...
    if (filter) {
      ...
      if (filter.fetchByWS) {
        alertsSubscription = WebsocketService.getInstance()
          .on<MarketStateDto>(
            filter.history
              ? WSEvent.HISTORY_MARKET_STATES
              : WSEvent.MARKET_STATES
          )
  ...
  const setDataReceivedFromServer = (marketState: MarketStateDto): void => {
    ...
    for (const item of marketState.marketStateIntervals[0].items) {
      ...
      const items = marketState.marketStateIntervals[1].items.filter(
        (value) =>
          value.candle.timestamp.getTime() >= item.candle.timestamp.getTime()
      );
      if (prevItemsLength !== items.length) {
        marketStateInterval.items = items.slice(
  ...
  const nameTemplate = (signal: Signal) => {
    ...
    return (
      <div key={signal.description} className={className} title={title}></div>
    );
  };
  ...
  const Row = ({ index, style }) => {
    ...
    const className =
      "market-state-column" +
      (selectedMarketStateItem === item ? " selected" : "");
    const classNameInner =
      "market-state-column-inner" +
      (selectedMarketStateItem === item ? " selected" : "");
    const innerWidthPercent =
      item.baseInterval === Interval.M3 ? 100 / limit : 100;
    ...
    return (
      <div className={index % 2 ? "ListItemOdd" : "ListItemEven"} style={style}>
        ...
        >
          <div
            className={
              "market-state-column-main " +
              item.baseItem.candleSentiment.toString().toLowerCase()
            }
          >
            <div className="market-state-column-time">
              {item.baseInterval} |{" "}
              {moment(item.baseItem.candle.timestamp).format(
                dateTimeFormat[item.baseInterval]
              )}
            </div>
            <div className="market-state-column-candle-sentiment">
              {candleSentimentMap[item.baseItem.candleSentiment]} /{" "}
              {item.baseItem.candle.close}
            </div>
            ...
            <div>
              {item.baseItem.signals
                .filter(
                  (signal) => signal.name.indexOf("CANDLE_PATTERN") !== -1
                )
            ...
            {item.items.map((value, index1) => {
              return (
                <div
                  key={index1}
                  className={
                    "market-state-column-inner-column " +
                    value.candleSentiment.toString().toLowerCase()
                  }
                ...
                >
                  <div className="market-state-column-time">
                    {moment(value.candle.timestamp).format(
                      dateTimeFormatInner[item.interval]
                    )}
                  </div>
                  <div className="market-state-column-candle-sentiment">
                    {candleSentimentMap[value.candleSentiment]} /{" "}
                    {value.candle.close}
                  </div>
                  <div className="market-state-column-ema-cross">
  ...
  return (
    <div className="p-grid market-state" style={{ height: viewHeight || 300 }}>
      <div
        className="p-col-12 market-state-body"
        style={{ height: (viewHeight || 300) - 0 }}
      >
        <AutoSizer>
          {({ height, width }) => (
            <List
