    ...
    if (filter) {
      ...
      if (filter.fetchByWS) {
        alertsSubscription = WebsocketService.getInstance()
          .on<SwingStateDto[]>(
            filter.history ? WSEvent.HISTORY_SWING_STATES : WSEvent.SWING_STATES
          )
          .subscribe((states) => {
  ...
  return (
    <div className="p-grid swing-state-list">
      <div className="p-col-12">
        {swingStates.map((swingState) => (
          <SwingState key={swingState.interval} swingState={swingState} />
        ))}
      </div>
    </div>
  );
