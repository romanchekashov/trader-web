...
export function getCandles(type: SecurityType, ticker: string, from: string, to: string, interval: Interval): Promise<Candle[]> {
    return fetch(`${baseUrl}candles?type=${type}&ticker=${ticker}&from=${from}&to=${to}&interval=${interval}`)
        .then(response => handleResponse(response)
            .then(adjustCandles))
        .catch(handleError);
}