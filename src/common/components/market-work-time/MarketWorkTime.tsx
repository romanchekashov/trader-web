...
const MarketWorkTime: React.FC<{}> = ({}) => {
    const CLOSED = 'closed'
    const OPEN = 'open'
    const [time, setTime] = useState(moment().format("DD-MM HH:mm:ss"))
    ...
    const updateMoex = () => {
        ...
        if (timeDay === 0 || timeDay === 6) {
            setMoex(CLOSED)
        } else if (timeInMinutes < START_IN_MINUTES || timeInMinutes > END_IN_MINUTES) {
            setMoex(CLOSED)
        } else if (timeInMinutes >= FIRST_CLEAR_START_IN_MINUTES && timeInMinutes <= FIRST_CLEAR_END_IN_MINUTES) {
            setMoex('mid clear')
        } else if (timeInMinutes >= SECOND_CLEAR_START_IN_MINUTES && timeInMinutes <= SECOND_CLEAR_END_IN_MINUTES) {
    ...
    const updateNewYorkAndChicagoStockExchange = () => {
        ...
        const timeDay = _time.day()
        if (timeDay > 0 && timeDay < 6
            && timeInMinutes >= START_IN_MINUTES && timeInMinutes <= END_IN_MINUTES) {
            setNyse(OPEN)
        } else {
            setNyse(CLOSED)
        }
    ...
    const update = () => {
        setTime(moment().format("HH:mm:ss/DD.MM"))
        updateMoex()
    ...
    return (
        <div className="market-work-time">
            ...
            <div style={{marginLeft: '5px', marginRight: '5px', display: 'flex'}}>{time}</div>
        </div>
    )
}
